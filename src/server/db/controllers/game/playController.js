  ...
  loadGame(req, res, next) {
    ...
    db.any(`SELECT card_id, card_name, mask, image, ebay_link, l.vintage, l.modern, l.standard, l.old_school, l.legacy, image_location_temp
            FROM "game.dbo".cards_n c
              ...
              WHERE  
          --case when MTG
                1 = CASE   WHEN g.game_name = 'MTG' and l.vintage IS NOT NULL AND $2 = 1 THEN 1 
                           WHEN g.game_name = 'MTG' and l.modern IS NOT NULL AND $3 = 1 THEN 1 
                           WHEN g.game_name = 'MTG' and l.standard IS NOT NULL AND $4 = 1 THEN 1 
                           WHEN g.game_name = 'MTG' and l.legacy IS NOT NULL AND $5 = 1 THEN 1 
                           WHEN g.game_name = 'MTG' and l.old_school IS NOT NULL AND $6 = 1 THEN 1 END
              ORDER BY RANDOM()
